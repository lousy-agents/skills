...
> **Prerequisite:** `plan-to-graph` requires a resolvable target repository and a GitHub surface that supports native sub-issues and blocking relationships (`gh issue create --parent`, `gh issue edit --add-blocked-by` locally, or the harness's GitHub MCP / built-in GitHub tools when `gh` is absent or lacks those flags). The skill probes before it creates anything and stops if no qualifying surface is found.
...
**Requires** a target GitHub repository and a GitHub surface that can create native sub-issues and blocking relationships — authenticated [`gh`](https://cli.github.com/) locally, or the harness's GitHub MCP / built-in GitHub tools when `gh` is absent or lacks those flags. Re-running against an epic that already has sub-issues will not duplicate them — the skill detects the collision and stops.
