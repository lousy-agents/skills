...
- Suggested shape: `given...` builds the starting state or fixture, `when...` performs the action under test, and `then...`/`assert...` checks the observable outcome. Keep each helper small, focused on one concern, and marked with `t.Helper()`.
